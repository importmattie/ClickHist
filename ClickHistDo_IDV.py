...
class ClickHistDo:
    def __init__(self, lons, lats, times,
        ...
        :param sessionLogFilename: The name of the file (without .ipynb
        ...
        # Set name of the session notebook log file
        self.sessionLogFilename = sessionLogFilename+'.ipynb'
    ...
    def do(self,flatIndex,**kwargs):
        ...
        if(os.path.exists('./Output/Scripts/') == False):
        ...
        if(os.path.isfile('./Output/SessionLogs/' +
                              self.sessionLogFilename) == False):
            print('Got here?')
            call('cp ./Output/Templates/sessionLogTemplate.ipynb ' +
                 './Output/SessionLogs/'+self.sessionLogFilename, shell=True)

        inFile = open('./Output/SessionLogs/' +
                     self.sessionLogFilename,'r')
        ...
        if(insertIndex == -1):
            print('ClickHistDo could not read the Session Log properly. ' +
                  'Not logging...')
        else:
            ...
            print cropCentX-centToEdgeX, cropCentY-centToEdgeY
            print centToEdgeX*2, centToEdgeY*2
            ...
            outFile = open('./Output/Tmp/'+self.sessionLogFilename,'w')
            outFile.writelines(lines)
            outFile.close()
            call('mv ./Output/Tmp/'+self.sessionLogFilename +
                 ' ./Output/SessionLogs/'+self.sessionLogFilename, shell=True)
